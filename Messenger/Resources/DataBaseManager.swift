...
extension DatabaseManager {
    ...
    public func getAllConversations(for email: String, completion: @escaping (Result<[Conversation], Error>) -> Void) {
        database.child("\(email)/conversations").observe(.value, with: { snapshot in
            ...
            let conversations: [Conversation] = value.compactMap({ dictionary in
                ...
                let latestMessageObject = LatestMessage(date:
                                                            date,
                                                        text: message,
                                                        isRead: isRead)
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                ...
                if type == "photo" {
                    ...
                    guard let longitude = Double(locationComponent[0]),
                        ...
                        print("failed to render location")
                        return nil
                    }
                    print("Rendering location \(longitude) and \(latitude)")
    ...
    public func sendMessage(to conversation: String, otherUserEmai: String, name: String, newMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        //update recepient latest messages
            ...
            strongSelf.database.child("\(conversation)/messages").setValue(currentMessages, withCompletionBlock: {error, _ in
                ...
                strongSelf.database.child("\(currentEmail)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                    ...
                    if var currentUserConversations = snapshot.value as? [[String: Any]] {
                        // создаем новый чат в списке чатов
                        ...
                        if var targetConversation = targetConversation {
                            ...
                            let newConversationData: [String: Any] = [
                                "id": conversation,
                                "other_user_email": DatabaseManager.safeEmail(emailAddress: otherUserEmai),
                                "name": name,
                    ...
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(databaseEntryConversations, withCompletionBlock: { error, _ in
                        ...
                        // показываем последнее сообщение в списке чатов
                        strongSelf.database.child("\(otherUserEmai)/conversations").observeSingleEvent(of: .value, with: { snapshot in
                            let updatedValue: [String: Any] = [
                                "date": dateString,
                                "is_read": false,
                                "message": message
                            ...
                            if var otherUserConversations = snapshot.value as? [[String: Any]] {
                                ...
                                // current collection doesn't exists
                                let newConversationData: [String: Any] = [
                                    "id": conversation,
                                    "other_user_email": DatabaseManager.safeEmail(emailAddress: currentEmail),
                                    "name": currentName,
                            ...
                            strongSelf.database.child("\(otherUserEmai)/conversations").setValue(databaseEntryConversations, withCompletionBlock: { error, _ in
                                guard error == nil else {
                                    completion(false)
                                    return
                                }
